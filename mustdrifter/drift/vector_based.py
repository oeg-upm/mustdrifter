...
def mmd_drift(reference_sample, test_sample, filename, K=100, n_jobs=10):
    """
    Perform Maximum Mean Discrepancy (MMD) drift detection between a
    reference sample and a test sample.

    This function estimates distributional drift using MMD with an RBF
    kernel and evaluates statistical significance through permutation
    testing. Results can be saved to a JSON file and intermediate results
    can be used to resume interrupted computations.
    ...
    Notes
    -----
    MMD is computed using an RBF kernel. Permutation testing is used to
    estimate whether the observed drift magnitude significantly differs from
    the null distribution generated by random resampling.
    """
...
def cos_drift(reference_sample, test_sample, filename, K=100, n_jobs=10):
    """
    Detect drift between a reference sample and a test sample using cosine
    distance.

    This function estimates semantic drift from the cosine distance between
    reference and test samples and evaluates statistical significance through
    permutation testing. Results can be exported to a JSON file.
    ...
    Notes
    -----
    Permutation testing is used to estimate whether the observed cosine
    drift magnitude significantly differs from the null distribution
    generated by random resampling.
    """
