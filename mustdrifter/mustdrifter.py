...
class MuSTDrifter:
    ...
    def calculate_drift(self, reference_sample, test_sample, filename, metrics, rebase=None):
        ...
        if "cos_drift" in metrics:
            ...
            if (rebase is True) or (os.path.exists(_filename) and os.path.exists(_filename.replace(".json", "_bak.json")) and rebase is None) or (not os.path.exists(_filename) and not os.path.exists(_filename.replace(".json", "_bak.json")) and rebase is False):
        ...
        for i in range(len(period_ids)-1):
            for e in range(len(period_ids)-1):
                ...
                if "sintactic" in drift_dimensions:
                    ...
                    self.calculate_sintactic_drift(reference_period=reference_period, test_period=test_period, metrics=_metrics, rebase=rebase)
