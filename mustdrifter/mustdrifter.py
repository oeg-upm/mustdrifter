...
class MuSTDrifter:
    ...
    def _calculate_drift(self, reference_sample, test_sample, filename, metrics, rebase=None):
        ...
        if "js_drift" in metrics:
            ...
            if filename is None: _filename = None
            if (rebase is True) or (os.path.exists(_filename) and os.path.exists(_filename.replace(".json", "_bak.json")) and rebase is None) or (not os.path.exists(_filename)) or (_filename is None):
